...
│      ▼                                                                          │
│   ┌─────────────────────────────────────────────────────────────────────────┐  │
│   │ 1. RECEIVE (REQ-CORE-003)                                               │  │
│   │    • Accept HTTP POST on :8080                                          │  │
│   │    • Parse JSON-RPC 2.0                                                 │  │
│   │    • Extract method, params, id                                         │  │
│   │    • Detect SEP-1686 task metadata                                      │  │
│   └───────────────────────────────────┬─────────────────────────────────────┘  │
│                                       │                                        │
│                                       ▼                                        │
│   ┌─────────────────────────────────────────────────────────────────────────┐  │
│   │ 2. ROUTE (REQ-CORE-003)                                                 │  │
│   │    • tools/call      → Policy Engine                                    │  │
│   │    • tasks/*         → Task Manager (SEP-1686)                          │  │
│   │    • resources/*     → Policy Engine                                    │  │
│   │    • prompts/*       → Policy Engine                                    │  │
│   │    • other           → Direct to Upstream                               │  │
│   └───────────────────────────────────┬─────────────────────────────────────┘  │
│                                       │                                        │
│                                       ▼                                        │
│   ┌─────────────────────────────────────────────────────────────────────────┐  │
...
│                                                                                 │
│   Priority 1 (Highest): ConfigMap mounted files                                 │
│                         /etc/thoughtgate/*.cedar                                │
│                         /etc/thoughtgate/config.yaml (future)                   │
│                                                                                 │
│   Priority 2:           Environment variables                                   │
│                         THOUGHTGATE_*                                           │
│                                                                                 │
│   Priority 3 (Lowest):  Compiled defaults                                       │
│                         (embedded in binary)                                    │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
|-----------------|--------|-------|
| **Gateway (centralized)** | ❌ Deferred | Requires agent authentication (API keys, mTLS, JWT) |
| **CLI wrapper** | ❌ Deferred | `thoughtgate wrap -- mcp-server` for local dev |
| **Embedded library** | ❌ Deferred | ThoughtGate as Rust crate |
...
| ThoughtGate → Slack | Webhook URL | URL acts as bearer token |
| Slack → ThoughtGate | HMAC-SHA256 | Signed callbacks prevent spoofing |
| Human → Slack | Slack auth | Slack handles user identity |
| ThoughtGate → Slack | Webhook URL (secret) |
| Slack → ThoughtGate | HMAC-SHA256 signature |
| Human → Slack | Slack authentication |
...
│        ▼                                                                        │
│   ┌─────────────────────────────────────────────────────────────────────────┐  │
│   │ REQ-CORE-003: Generate correlation_id (UUID v4)                         │  │
│   │              Store in McpRequest.correlation_id                         │  │
│   └─────────────────────────────────────────────────────────────────────────┘  │
│        │                                                                        │
│        ├─────────────────────────────────────────────────────────────────┐      │
│        │                                                                 │      │
│        ▼                                                                 ▼      │
│   ┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌───────────────┐   │
│   │  POL-001    │    │  CORE-001   │    │  CORE-002   │    │   GOV-001     │   │
│   │             │    │             │    │             │    │               │   │
│   │ Log with    │    │ Log with    │    │ Log with    │    │ task_id +     │   │
...
**⚠️ v0.1 uses blocking mode, NOT SEP-1686 auto-upgrade.**
...
│                                                                                 │
│   ThoughtGate behavior:                                                         │
│   1. Holds HTTP connection open                                                │
│   2. Sends approval request to Slack                                           │
│   3. Waits for human response (with timeout)                                   │
│   4a. On APPROVE: Execute tool, return normal result                           │
│   4b. On REJECT: Return error -32007 (ApprovalRejected)                        │
│   4c. On TIMEOUT: Return error -32008 (ApprovalTimeout)                        │
│                                                                                 │
│   From client perspective: Slow tool call, but standard response               │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
See RFC-001 §10.1 for full SEP-1686 compliance plan.
