...
async fn test_100mb_stream_memory_profile() {
    ...
    tokio::time::timeout(
        ...
        ready_rx
    ).await.expect("Relay failed to start within timeout").expect("Relay startup failed");
    ...
    println!("Total received: {} bytes ({:.2} MB)", total_received, total_received as f64 / 1_048_576.0);
    println!("Time elapsed: {:.2}s", elapsed.as_secs_f64());
    println!("Throughput: {:.2} MB/s", (total_received as f64 / 1_048_576.0) / elapsed.as_secs_f64());
    println!("\n=== Memory Verification ===");
    ...
    assert_eq!(total_received, 100 * 1024 * 1024, "Should receive exactly 100MB");
    assert_eq!(bytes_sent.load(Ordering::SeqCst), 100 * 1024 * 1024, "Origin should send 100MB");
    assert_eq!(bytes_relayed.load(Ordering::SeqCst), 100 * 1024 * 1024, "Relay should forward 100MB");
}
...
async fn test_memory_bounded_by_buffer() {
    ...
    println!("✅ Streamed {}MB using only {}KB buffer", 
             STREAM_SIZE / 1_048_576, BUFFER_SIZE / 1024);
}
